...
bool RaceSanitizer::DoesDcciHaveEffect(SanEvent const &event, DcciRecord const &dcciRecord) const {
    ...
    if (deviceType_ > DeviceType::ASCEND_910B_START && deviceType_ < DeviceType::ASCEND_910B_END) {
        cacheLineSize = 256;
    } else if (deviceType_ > DeviceType::ASCEND_950_START && deviceType_ < DeviceType::ASCEND_950_END) {
        cacheLineSize = 64;
    } else {
