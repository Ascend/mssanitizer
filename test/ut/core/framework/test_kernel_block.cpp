...
TEST_F(TestKernelBlock, CreateKernelBlock_ShadowMemoryInRange)
{
    ...
    ASSERT_NE(kernelBlock, nullptr);
    EXPECT_NE(kernelBlock->shadowMemoryHead_, nullptr);
}
...
TEST_F(TestKernelBlock, CreateKernelBlock_ShadowMemoryOutOfRange)
{
    ...
    ASSERT_NE(kernelBlock, nullptr);
    // shadowMemoryHead_ 应该为 nullptr，因为超出了范围
    EXPECT_EQ(kernelBlock->shadowMemoryHead_, nullptr);
}
...
// 测试 CreateKernelBlock simtEntry 超出 memSize
TEST_F(TestKernelBlock, CreateKernelBlock_SimtEntryOutOfRange)
{
    uint64_t memSize = 0;
    auto buffer = CreateValidMemBuffer(memSize, 0);

    // 修改 recordCount 使 simtEntry 数据超出范围
    // 从 buffer 中找到 ShadowMemoryRecordHead 的位置并修改 recordCount
    // ShadowMemoryRecordHead 在 SIMT 区域之后
    uint64_t simdRecordsSize = 1024;
    uint64_t simtErrorInfoSize = sizeof(SimtRecordBlockHead);
    uint64_t simtSize = (simtErrorInfoSize + sizeof(SimtRecordBlockHead)) * SIMT_THREAD_MAX_SIZE;
    uint64_t shadowMemOffset = sizeof(RecordGlobalHead) + sizeof(RecordBlockHead) + simdRecordsSize + simtSize;
    auto *shadowHead = reinterpret_cast<ShadowMemoryRecordHead *>(buffer.data() + shadowMemOffset);
    shadowHead->recordCount = 0xFFFFFFFF; // 设置一个很大的值
